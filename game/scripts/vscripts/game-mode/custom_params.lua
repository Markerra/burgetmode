CUSTOM_FOUNTAIN_VUL_DELAY 	= 600 -- время до деактивации неуязвимости фонтанов (в секундах) // 10 мин

CUSTOM_START_LEVEL          = 3
CUSTOM_START_GOLD		  	= 600
CUSTOM_WARD_STOCK_COUNT     = 8
MAXIMUM_ATTACK_SPEED		= 800
COURIER_MAX 				= true
CUSTOM_BUYBACK_COST_ENABLED = false
CUSTOM_BUYBACK_COST         = 0

CUSTOM_FOUNTAIN_AURA_RADIUS    = 675 -- радиус действия ауры лечения у фонтана 
CUSTOM_FOUNTAIN_PCT_HP_REGEN   = 4.5 -- регенерация здоровья около фонтана (%)
CUSTOM_FOUNTAIN_PCT_MANA_REGEN = 6 -- регенерация маны около фонтана (%)
CUSTOM_FOUNTAIN_PCT_DMG_RESIST = 8 -- доп. сопротивление урону около фонтана (%)

CUSTOM_ALLOWED_STEAMIDS = { -- стим айди разрешенные для админ панели
    "76561199052097407",
    "76561199168798322",
}

CUSTOM_DEBUG_MODE = true
CUSTOM_FORCE_HERO = "legion_commander"